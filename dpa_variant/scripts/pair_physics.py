#!/usr/bin/env python3
"""Layer 1: per-pair physical residual on top of a ready MACE-MH-0 (WITHOUT retraining).

Correction = missing ZBL repulsion, RELATIVE to the model's own dimer:
...
- subtract the model's OWN dimer: MACE-MH-0 already has built-in ZBL → we add only what is MISSING.
- f_cut → 0 above r_cut = κ·(r_cov_i+r_cov_j) → self-zeroing at the equilibrium bond (valid bonds untouched).

DimerCache computes the model curve on a grid lazily (only for encountered pairs) and caches it.
"""
...
class DimerCache:
    """Lazy per-pair cache of residual ΔV(r) and dΔV/dr (via the model's own dimer)."""
    ...
    def get(self, Zi, Zj):
        ...
        fcut = _smoothstep(rgrid, r_cut - self.width, r_cut)  # self-zeroing at equilibrium
        dV = residual * fcut
    ...
    def save(self, path=None):
        """Save the dimer-curve table to disk (pickle). Called after precompute."""
        import pickle
        from pathlib import Path as _P
